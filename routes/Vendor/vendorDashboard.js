...
router.get('/show-product',(req,res)=>{
    ...
    (select b.name from brand b where b.id = p.brandid) as brandname
     from products p where p.vendorid = '${req.session.vendorid}' order by id desc`,(err,result)=>{
...
router.post('/update/vendor/details', (req, res) => {
   ...
   console.log(req.body)
    ...
    router.get('/coupon-show',(req,res)=>{
        pool.query(`select p.* from coupon p where p.vendorid = '${req.session.vendorid}'`,(err,result)=>{
            err ? console.log(err) : res.json(result)
        })
    })
